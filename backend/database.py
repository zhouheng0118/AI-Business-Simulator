...
def _is_semantic_duplicate(existing: dict, candidate: dict) -> bool:
    ...
    return len(overlap) / smaller_size >= 0.70
...
def update_playbook_content(playbook_id: str, fields: dict) -> None:
    result = (
        _get_client().table("playbooks").update(fields).eq("id", playbook_id).execute()
    )
    if not result.data:
        raise RuntimeError(f"Playbook {playbook_id} not found during content update")
